...
public final class FluidTexture {
  ...
  /**
   * @deprecated use
   *             {@link #FluidTexture(ResourceLocation, ResourceLocation, ResourceLocation, ResourceLocation, float, int, int, boolean, FogShape, float, float)}
   */
  @Deprecated(forRemoval = true)
  public FluidTexture(ResourceLocation still, ResourceLocation flowing, @Nullable ResourceLocation overlay,
      @Nullable ResourceLocation camera, int color) {
  ...
  public JsonObject serialize() {
    ...
    // during datagen, we just write the texture directly, we will include the
    // needed prefix/suffix on read
    if (camera != null) {
  ...
  public static FluidTexture deserialize(JsonObject json) {
    ...
    int fogColor = color | 0xFF000000; // default fog color to opaque variant of fluid color. If no tint this will end
                                       // up as -1
  ...
  public static class Builder {
     ...
     * @param suffix  Suffix for placing textures, included before "still" or
     *                "flowing". Typically will want "/" or "_".
    ...
    public Builder wrapId(String prefix, String suffix, boolean overlay, boolean camera) {
      return textures(
          JsonHelper.wrap(Objects.requireNonNull(NeoForgeRegistries.FLUID_TYPES.get().getKey(fluid)), prefix, suffix),
          overlay, camera);
    }
     ...
     * @param camera  If true, include a camera texture
     * @return Builder instance
     * @deprecated use {@link #root(ResourceLocation)}, {@link #still()},
     *             {@link #flowing()}, {@link #camera()}, and {@link #overlay()}
     */
    @Deprecated
    ...
    public FluidTexture build() {
      ...
      return new FluidTexture(still, flowing, overlay, camera, cameraOpacity, color, fogColor,
          fogColor == -1 && calculateFogColor, fogShape, fogStart, fogEnd);
    }
