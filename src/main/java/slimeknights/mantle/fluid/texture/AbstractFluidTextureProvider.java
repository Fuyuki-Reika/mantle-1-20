...
public abstract class AbstractFluidTextureProvider extends GenericDataProvider {
  ...
  public final CompletableFuture<?> run(CachedOutput cache) {
    ...
    if (modId != null) {
      List<String> missing = fluidTypeRegistry.entrySet().stream()
          .filter(entry -> entry.getKey().location().getNamespace().equals(modId)
              && !allTextures.containsKey(entry.getValue()) && !ignore.contains(entry.getValue()))
    ...
    // save files
    return allOf(allTextures.entrySet().stream().map(entry -> saveJson(cache,
        Objects.requireNonNull(fluidTypeRegistry.getKey(entry.getKey())), entry.getValue().build().serialize())));
  }
  ...
  /**
   * Gets the map of all textures. Shouod not be called in {@link #addTextures()},
   * meant for other data generators to use.
   */
