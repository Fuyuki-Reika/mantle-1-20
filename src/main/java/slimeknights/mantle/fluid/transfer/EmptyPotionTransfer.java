...
public class EmptyPotionTransfer extends EmptyFluidContainerTransfer {
  ...
  public boolean matches(ItemStack stack, FluidStack fluid) {
    ...
    return super.matches(stack, fluid)
      && (TagPreference.getPreference(MantleTags.Fluids.POTION).isPresent() || stack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).potion().map(holder -> holder.value()).orElse(Potions.EMPTY) == Potions.WATER);
  }

  @Override
  protected FluidStack getFluid(ItemStack stack) {
    // water just returns water
    if (stack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).potion().map(holder -> holder.value()).orElse(Potions.EMPTY) == Potions.WATER) {
      return fluid.copy();
    }
