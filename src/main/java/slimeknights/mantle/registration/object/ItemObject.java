...
public class ItemObject<I extends ItemLike> implements Supplier<I>, ItemLike, IdAwareObject {
  ...
  /**
   * Registry name for this entry, allows fetching the name before the entry
   * resolves if registry object is used
   */
  @Getter
  private final ResourceLocation id;

  /**
   * Creates a new item object from a supplier instance. Registry name will be
   * fetched from the supplier entry, so the entry must be present during
   * construction
   * 
   * @param entry Existing registry entry, typically a vanilla block or a
   *              registered block
   */
  ...
  /**
   * Creates a new item object using the given registry object. This variant can
   * resolve its name before the registry object entry resolves
   * 
  ...
  /**
   * Creates a new item object using another item object. Intended to be used in a
   * subclass to avoid an extra wrapper
   * 
   * @param object Object base
   */
