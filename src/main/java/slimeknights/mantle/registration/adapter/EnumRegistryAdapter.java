...
public class EnumRegistryAdapter<T> extends RegistryAdapter<T> {
  ...
  /**
   * Registers an entry with multiple variants, prefixing the name with the value
   * name
   * 
   ...
   */
  public <E extends Enum<E> & StringRepresentable, I extends T> EnumObject<E, I> registerEnum(Function<E, I> mapper,
      E[] values, String name) {
    if (values.length == 0) {
  ...
  /**
   * Registers an entry with multiple variants, suffixing the name with the value
   * name
   * 
   * @param mapper Function to get a block for the given enum value
   * @param name   Name of the block
