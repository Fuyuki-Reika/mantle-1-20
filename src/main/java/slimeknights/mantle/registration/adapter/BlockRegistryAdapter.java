...
public class BlockRegistryAdapter extends EnumRegistryAdapter<Block> {
    ...
    public <T extends Block> T registerOverride(Function<Properties, T> constructor, Block base) {
        return register(constructor.apply(Properties.ofFullCopy(base)), base);
    }
     ...
     * Uses the vanilla slab and stair blocks. Uses the passed blocks properties for
     * both.
     ...
     * @param name  The registry name to use for the block and as base for the slab
     *              and stairs
    ...
    public BuildingBlockObject registerBuilding(Block block, String name) {
        return new BuildingBlockObject(
                ...
                this.register(new SlabBlock(Properties.ofFullCopy(block)), name + "_slab"),
                this.register(new StairBlock(block.defaultBlockState(), Properties.ofFullCopy(block)),
                        name + "_stairs"));
    }

    /**
     * Same as {@link #registerBuilding(Block, String)}, but also includes a wall
     * variant
     *
     * @param block The main block to register and whose properties to use
     * @param name  The registry name to use for the block and as base for the slab
    ...
    public WallBuildingBlockObject registerWallBuilding(Block block, String name) {
        return new WallBuildingBlockObject(
                ...
                this.register(new WallBlock(Properties.ofFullCopy(block)), name + "_wall"));
    }

    /**
     * Same as {@link #registerBuilding(Block, String)}, but also includes a fence
     * variant
     *
     * @param block The main block to register and whose properties to use
     * @param name  The registry name to use for the block and as base for the slab
    ...
    public FenceBuildingBlockObject registerFenceBuilding(Block block, String name) {
        return new FenceBuildingBlockObject(
                registerBuilding(block, name),
                this.register(new FenceBlock(Properties.ofFullCopy(block)), name + "_fence"));
    }
    ...
    public WoodBlockObject registerWood(String name, Function<WoodVariant, BlockBehaviour.Properties> behaviorCreator) {
        ...
        // planks
        BlockBehaviour.Properties planksProps = behaviorCreator.apply(WoodVariant.PLANKS)
        ...
        FenceBlock fence = register(new FenceBlock(Properties.ofFullCopy(planks.get()).forceSolidOn()),
                name + "_fence");
        // logs and wood
        Supplier<? extends RotatedPillarBlock> stripped = () -> new RotatedPillarBlock(
                behaviorCreator.apply(WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).strength(2.0f));
        ...
        RotatedPillarBlock log = register(new StrippableLogBlock(getHolder(BuiltInRegistries.BLOCK, strippedLog),
                behaviorCreator.apply(WoodVariant.LOG).instrument(NoteBlockInstrument.BASS).strength(2.0f)),
                name + "_log");
        RotatedPillarBlock wood = register(
                new StrippableLogBlock(getHolder(BuiltInRegistries.BLOCK, strippedWood),
                        behaviorCreator.apply(WoodVariant.WOOD).instrument(NoteBlockInstrument.BASS).strength(2.0f)),
                name + "_wood");

        // doors
        DoorBlock door = register(new DoorBlock(setType, behaviorCreator.apply(WoodVariant.PLANKS)
                .instrument(NoteBlockInstrument.BASS).strength(3.0F).noOcclusion().pushReaction(PushReaction.DESTROY)),
                name + "_door");
        TrapDoorBlock trapdoor = register(new TrapDoorBlock(setType, behaviorCreator.apply(WoodVariant.PLANKS)
                .instrument(NoteBlockInstrument.BASS).strength(3.0F).noOcclusion().isValidSpawn(Blocks::never)),
                name + "_trapdoor");
        FenceGateBlock fenceGate = register(new FenceGateBlock(woodType, Properties.ofFullCopy(fence)),
                name + "_fence_gate");
        // redstone
        BlockBehaviour.Properties redstoneProps = behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn()
                .instrument(NoteBlockInstrument.BASS).noCollission().pushReaction(PushReaction.DESTROY).strength(0.5F);
        PressurePlateBlock pressurePlate = register(new PressurePlateBlock(setType, redstoneProps),
                name + "_pressure_plate");
        ButtonBlock button = register(new ButtonBlock(setType, 30, redstoneProps), name + "_button");
        // signs
        StandingSignBlock standingSign = register(new MantleStandingSignBlock(behaviorCreator.apply(WoodVariant.PLANKS)
                .forceSolidOn().instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F), woodType),
                name + "_sign");
        WallSignBlock wallSign = register(
                new MantleWallSignBlock(behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn()
                        .instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F).dropsLike(standingSign),
                        woodType),
                name + "_wall_sign");
        MantleCeilingHangingSignBlock hangingSign = register(
                new MantleCeilingHangingSignBlock(behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn()
                        .instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F), woodType),
                name + "_hanging_sign");
        MantleWallHangingSignBlock wallHangingSign = register(
                new MantleWallHangingSignBlock(behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn()
                        .instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F).dropsLike(hangingSign),
                        woodType),
        ...
        // finally, return
        return new WoodBlockObject(getResource(name), woodType, planks, log, strippedLog, wood, strippedWood, fence,
                fenceGate, door, trapdoor, pressurePlate, button, standingSign, wallSign, hangingSign, wallHangingSign);
    }
     ...
     * @param name       Fluid name, unfortunately no way to fetch from the fluid as
     *                   it does not exist yet
     * @return Fluid block instance
     */
    public LiquidBlock registerFluidBlock(Supplier<? extends BaseFlowingFluid> fluid, MapColor color, int lightLevel,
            String name) {
        return register(new LiquidBlock(fluid.get(), FluidDeferredRegister.createProperties(color, lightLevel)),
                name + "_fluid");
    }
}
