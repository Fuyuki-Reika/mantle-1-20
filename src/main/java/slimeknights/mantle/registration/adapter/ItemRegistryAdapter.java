...
public class ItemRegistryAdapter extends EnumRegistryAdapter<Item> {
   ...
   */
  public ItemRegistryAdapter(Registry<Item> registry) {
    this(registry, null);
  }
   ...
   */
  public ItemRegistryAdapter(Registry<Item> registry, @Nullable Item.Properties defaultProps) {
    super(registry);
   ...
   * @param defaultProps Default item properties
   */
  public ItemRegistryAdapter(Registry<Item> registry, String modid, @Nullable Item.Properties defaultProps) {
   ...
   * If your block does not have its own item, just use this method to make it
   * available as an item.
   * The item uses the same name as the block for registration.
   * The registered BlockItem has tooltip support by default, see
   * {@link BlockTooltipItem}
   * It will be added to the creative itemgroup passed in in the constructor. If
   * you want a different one, use the method with a ItemGroup parameter.
   *
  ...
  /**
   * Same as the variant without ItemGroup, but registers it for the given
   * itemgroup.
   * 
   * @param block The block you want to have an item for
  ...
  /**
   * Shortcut method to register your own BlockItem, registering with the same
   * name as the block it represents.
   * 
  ...
  public void registerDefaultBlockItem(WoodBlockObject object, boolean isBurnable) {
    ...
    // sign
    registerBlockItem(
        burnableSignItem.apply(new Item.Properties().stacksTo(16), object.getSign(), object.getWallSign()));
    registerBlockItem(burnableHangingSignItem.apply(new Item.Properties().stacksTo(16), object.getHangingSign(),
        object.getWallHangingSign()));
  }
   ...
   */
  public <B extends Block> void registerBlockItem(EnumObject<?, B> enumObject,
      Function<B, ? extends BlockItem> blockItem) {
   ...
   * @param baseName Fluid name, unfortunately cannot be fetched from the fluid as
   ...
   * @param baseName  Entity name, as it may or may not be present in the entity
   *                  type
