...
/**
 * Adds fluid transfer for base fluid. Use
 * {@link AbstractFluidContainerTransferProvider} for mods.
 */
@Internal
public class MantleFluidTransferProvider extends AbstractFluidContainerTransferProvider {
  ...
  protected void addTransfers() {
    addTransfer("wet_sponge", new FillFluidContainerTransfer(Ingredient.of(Items.SPONGE),
        ItemOutput.fromItem(Items.WET_SPONGE), FluidIngredient.of(MantleTags.Fluids.WATER, MantleValues.BOTTLE)));
    ...
    // foods
    optionalFillEmpty("honey_bottle_", Items.HONEY_BOTTLE, Items.GLASS_BOTTLE, MantleTags.Fluids.HONEY,
        MantleValues.BOTTLE, false);
    optionalFillEmpty("beetroot_soup_", Items.BEETROOT_SOUP, Items.BOWL, MantleTags.Fluids.BEETROOT_SOUP,
        MantleValues.BOWL, false);
    optionalFillEmpty("mushroom_stew_", Items.MUSHROOM_STEW, Items.BOWL, MantleTags.Fluids.MUSHROOM_STEW,
        MantleValues.BOWL, false);
  ...
  protected void addPotion(String prefix, ItemLike filled, @Nullable TagKey<Item> bottleTag) {
    ...
    if (bottleTag != null) {
      ...
      // since the container tag may not be present, add two potion recipes: one when
      // its absent that gives glass bottle, and one when present for unique bottle
      addTransfer(prefix + "empty_glass_bottle",
          new EmptyPotionTransfer(Ingredient.of(filled), ItemOutput.fromItem(Items.GLASS_BOTTLE), MantleValues.BOTTLE),
          new NotCondition(containerCondition));
      addTransfer(prefix + "empty_unique_bottle",
          new EmptyPotionTransfer(Ingredient.of(filled), ItemOutput.fromTag(bottleTag), MantleValues.BOTTLE),
          containerCondition);
    } else {
    ...
    // filling potions is as simple as a NBT copy, though this requires a potion
    // fluid and possibly a container
