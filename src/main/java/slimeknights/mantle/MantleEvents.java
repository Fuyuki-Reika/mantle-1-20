...
public class MantleEvents {
  /* Soulbound */
  /**
   * NBT key for items to preserve their slot in soulbound. Applied to items
   * tagged {@link MantleTags.Items#SOULBOUND}.
   * May be used by dependencies mods in {@link LivingDeathEvent} to make items
   * soulbound for other reasons.
   */
  ...
  static void onLivingDeath(LivingDeathEvent event) {
    // this is the latest we can add slot markers to the items so we can return them
    // to slots
    LivingEntity entity = event.getEntity();
    if (!entity.level().getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY) && entity instanceof Player player
  ...
  /**
   * Called when the player dies to store the soulbound items in the original
   * inventory
   */
  ...
  static void onPlayerDropItems(LivingDropsEvent event) {
    ...
    if (!entity.level().getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY) && entity instanceof Player player
      ...
      while (iter.hasNext()) {
        ...
        if (!customData.isEmpty() && customData.contains(SOULBOUND_SLOT)) {
          ...
          if (inventory.getItem(slot).isEmpty()) {
            ...
            // want to make sure they don't get in the way of items that have not yet been
            // seen
      ...
      // handle items that did not get their requested slot last, to ensure they don't
      // take someone else's slot while being added to a default
      for (ItemEntity itemEntity : takenSlot) {
        ItemStack stack = itemEntity.getItem();
        if (!inventory.add(stack)) {
          // last resort, somehow we just cannot put the stack anywhere, so drop it on the
          // ground
  ...
  /**
   * Called when the new player is created to fetch the soulbound item from the
   * old
   */
  @SubscribeEvent(priority = EventPriority.HIGH)
  static void onPlayerClone(PlayerEvent.Clone event) {
    ...
    int size = Math.min(originalInv.getContainerSize(), cloneInv.getContainerSize()); // not needed probably, but might
                                                                                      // as well be safe
