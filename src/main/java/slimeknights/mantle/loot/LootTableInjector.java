...
public enum LootTableInjector implements IEarlyReloadListener {
  ...
  public static void init() {
    ...
    MinecraftForge.EVENT_BUS.addListener(EventPriority.NORMAL, false, LootTableLoadEvent.class,
  ...
  public void onResourceManagerReload(ResourceManager manager) {
    ...
    int loaded = 0;
    for (Entry<ResourceLocation, Resource> entry : manager.listResources(FOLDER, loc -> loc.getPath().endsWith(".json"))
        .entrySet()) {
    ...
    // log timer
    Mantle.logger.info("Loaded {} loot table injectors injecting into {} tables in {} ms", loaded, injections.size(),
