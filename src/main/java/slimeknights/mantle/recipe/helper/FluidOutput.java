...
/**
 * Class representing a fluid stack output. Supports both direct stacks and tag
 * output, behaving like
 * {@link slimeknights.mantle.recipe.ingredient.FluidIngredient} used for output
 */
public abstract class FluidOutput implements Supplier<FluidStack> {
  ...
  /**
   * Gets the size of the fluid. Equivelent to {@link #get()} then
   * {@link FluidStack#getAmount()} but saves resolving the fluid.
   */
  ...
  public enum Loadable implements RecordLoadable<FluidOutput> {
    ...
    public <P> LoadableField<FluidOutput, P> emptyField(String key, boolean serializeDefault,
        Function<P, FluidOutput> getter) {
