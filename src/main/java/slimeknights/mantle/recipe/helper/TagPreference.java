...
public class TagPreference {
  ...
  /**
   * Cache of comparator instances, concurrent as hash map optimizations means the
   * whole compute if absent method is not entirely synchronized
   */
  ...
  /**
   * Gets the preference from a tag without going through the cache, internal
   * logic behind {@link #getPreference(TagKey)}
   */
  private static <T> Optional<T> getUncachedPreference(TagKey<T> tag) {
    ...
    // streams have a lovely function to get the minimum element based on a
    // comparator
  ...
  /**
   * Don't create a new lambda instance every time we call
   * {@link #getPreference(TagKey)}
   */
  ...
  /**
   * Special casing comparator to ensure flowing fluids are sorted last after
   * fluid sources.
   */
