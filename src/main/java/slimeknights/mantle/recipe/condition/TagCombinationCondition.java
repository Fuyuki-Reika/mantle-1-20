 ...
 * @param ignore Entries in this tag will be ignored towards the match. If null,
...
public record TagCombinationCondition<T>(List<TagKey<T>> match, @Nullable TagKey<T> ignore) implements ICondition {
  ...
  public boolean test(IContext context) {
    ...
    itemLoop: for (Holder<T> entry : tags.get(0)) {
  ...
  public static class TagCombinationConditionSerializer {
    ...
    public TagCombinationCondition<?> read(JsonObject json) {
      ...
      ResourceKey<Registry<Object>> registry = ResourceKey
          .createRegistryKey(JsonHelper.getResourceLocation(json, "registry", Registries.ITEM.location()));
