...
public class MantleCommand {
  ...
  /**
   * Permission level that can run standard game commands, used by command blocks
   * and functions
   */
  ...
  /**
   * Permission level for the server owner, server console, or the player in
   * single player
   */
  ...
  /**
   * @deprecated use {@link RegistryArgument#VALUE} or
   *             {@link TagSourceArgument#VALUE}
   */
  @Deprecated(forRemoval = true)
  public static SuggestionProvider<CommandSourceStack> REGISTRY_VALUES;
  /**
   * @deprecated use {@link RegistryArgument#REGISTRY} or
   *             {@link TagSourceArgument#SOURCE}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * Returns true if the source either does not have reduced debug info or they
   * have the proper level
   * Allows limiting a command that prints debug info to not work in reduced debug
   * info
   * 
  ...
  public static boolean requiresDebugInfoOrOp(CommandSourceStack source, int reducedDebugLevel) {
    return !source.getLevel().getGameRules().getBoolean(GameRules.RULE_REDUCEDDEBUGINFO)
        || source.hasPermission(reducedDebugLevel);
  }
}
