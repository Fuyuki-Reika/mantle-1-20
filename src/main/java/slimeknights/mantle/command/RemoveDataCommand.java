...
public class RemoveDataCommand {
  /* Name is invalid */
  private static final Dynamic2CommandExceptionType INVALID_REGISTRY = new Dynamic2CommandExceptionType(
      (name, registry) -> Mantle.makeComponent("command", "key.wrong_registry", name, registry));
  // success
  /** Translation key for successfully removing structure sets */
  private static final String STRUCTURE_SET_SUCCESS = Mantle.makeDescriptionId("command",
      "remove_data.structure.success");
  /** Translation key for successfully removing biome modifiers */
  private static final String BIOME_MODIFIER_SUCCESS = Mantle.makeDescriptionId("command",
      "remove_data.biome_modifier.success");
  ...
  private static <T> ResourceKey<T> getResourceKey(CommandContext<CommandSourceStack> context, String name,
      ResourceKey<Registry<T>> registry) throws CommandSyntaxException {
  ...
  private static int removeStructureSet(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
    ...
    ResourceLocation setLocation = JsonHelper.wrap(id.location(), Registries.STRUCTURE_SET.location().getPath() + '/',
        ".json");
    ...
    // save the final JSON
    Path path = pack.resolve(PackType.SERVER_DATA.getDirectory())
        .resolve(setLocation.getNamespace() + '/' + setLocation.getPath());
    ...
    context.getSource().sendSuccess(() -> Component.translatable(STRUCTURE_SET_SUCCESS, id.location(), time,
  ...
  private static int removeBiomeModifier(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
    ...
    // start by fetching the existing structure set JSON
    ResourceLocation modifierLocation = JsonHelper.wrap(id.location(),
        BIOME_MODIFIERS.location().getNamespace() + '/' + BIOME_MODIFIERS.location().getPath() + '/', ".json");
    ...
    Path path = pack.resolve(PackType.SERVER_DATA.getDirectory())
        .resolve(modifierLocation.getNamespace() + '/' + modifierLocation.getPath());
    ...
    // send success
    float time = (System.nanoTime() - startTime) / 1000000f;
    context.getSource().sendSuccess(() -> Component.translatable(BIOME_MODIFIER_SUCCESS, id.location(), time,
        GeneratePackHelper.getOutputComponent(pack)), true);
    return 1;
  }
}
