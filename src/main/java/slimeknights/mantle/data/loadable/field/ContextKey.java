...
/**
 * Key for fetching properties from a loadable context. This key doubles as a
 * record field for a required context key.
 * 
 * @param <T> Field type
 */
...
public class ContextKey<T> implements Key<T> {
  ...
  /**
   * Key for adding condition context, used in
   * {@link slimeknights.mantle.data.loadable.mapping.ConditionalLoadable}
   */
  ...
  /**
   * Gets a field requiring this context parameter, but mapped using the passed
   * function
   */
  ...
  private record Required<T, M>(ContextKey<T> key, BiFunction<T, ErrorFactory, M> mapper) implements ContextField<M> {
    ...
    public M get(TypedMap context, ErrorFactory error) {
      ...
      throw error
          .create("Unable to fetch " + key.name + " from context, this usually implements a broken JSON deserializer");
    }
  }
  ...
  private record Defaulting<T>(ContextKey<T> key, @Nullable T defaultValue) implements ContextField<T> {
    ...
    public T get(TypedMap context, ErrorFactory error) {
      // it is potentially faster to use get over getOrDefault so call it directly if
