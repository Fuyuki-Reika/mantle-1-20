 ...
 * Automatically used in {@link GenericLoaderRegistry} for all types, though you
 * will have to manually provide datagen using {@link ConditionalObject}.
 * 
 * @param registry       Loader registry to fetch nested objects.
 * @param defaultIfFalse Value to use if the condition fails and no false object
 *                       is provided, typically should be an empty object. If
 *                       null, a false object must be specified.
 */
public record ConditionalLoadable<T extends IHaveLoader>(GenericLoaderRegistry<T> registry, @Nullable T defaultIfFalse)
  ...
  public T decode(FriendlyByteBuf buffer, TypedMap context) {
    throw new UnsupportedOperationException(
        "Conditional loadable should always resolve to a specific instance. This should never happen.");
  }
