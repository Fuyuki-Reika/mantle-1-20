...
public class FluidStackLoadable {
  ...
  // TODO 1.21.1: hasTag() removed, check if components patch is empty instead
  ...
  // TODO 1.21.1: getTag() removed - NBT field temporarily disabled
  // Data component migration needed - FluidStack no longer supports CompoundTag
  // NBT
  // private static final LoadableField<CompoundTag,FluidStack> NBT =
  // NBTLoadable.ALLOW_STRING.nullableField("nbt", stack -> {
  // // Convert ComponentsPatch to CompoundTag for legacy format
  // CompoundTag tag = new CompoundTag();
  // stack.getComponentsPatch().save(tag);
  ...
  // public static final RecordLoadable<FluidStack> OPTIONAL_BUCKET_NBT =
  // fixedSizeNBT(FluidType.BUCKET_VOLUME);
  ...
  // public static final RecordLoadable<FluidStack> REQUIRED_BUCKET_NBT =
  // notEmpty(OPTIONAL_BUCKET_NBT);
  /** Loadable for a stack that may not be empty with variable count and NBT */
  // public static final RecordLoadable<FluidStack> REQUIRED_STACK_NBT =
  ...
  private static FluidStack makeStack(Fluid fluid, int amount) {
    ...
    // TODO 1.21.1: FluidStack(Fluid, int, CompoundTag) constructor removed
  ...
  // TODO 1.21.1: fixedSizeNBT disabled until data component migration complete
  /** Creates a loadable for a stack with a single item */
  /*
   * public static RecordLoadable<FluidStack> fixedSizeNBT(int amount) {
   * if (amount <= 0) {
   * throw new IllegalArgumentException("Amount must be positive, received " +
   * amount);
   * }
   * return RecordLoadable.create(FLUID, NBT, (fluid, tag) -> makeStack(fluid,
   * amount, tag))
