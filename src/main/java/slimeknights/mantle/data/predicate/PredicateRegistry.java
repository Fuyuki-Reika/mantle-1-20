...
/**
 * Extension of generic loader registry providing default implementations for
 * common predicates
 */
public class PredicateRegistry<T> extends DefaultingLoaderRegistry<IJsonPredicate<T>> {
   ...
   * @param anyInstance  Any instance that always returns true. Will be used for
   *                     nulls and missing fields.
   * @param noneInstance None instance that always returns false. Used as the
   *                     default for the conditional loader. If null, no none is
   *                     registered.
   */
  ...
  /**
   * @deprecated use
   *             {@link #PredicateRegistry(String, IJsonPredicate, IJsonPredicate)}
   */
  ...
  /**
   * Predicate that based on a load condition will use change the predicate
   * loaded.
   * 
