...
public class OffhandCooldownTracker implements ICapabilityProvider {
  ...
  /**
   * Enables the cooldown tracker if above 0. Intended to be set in equipment
   * change events, not serialized
   */
  ...
  /**
   * Call this method when your item causing offhand cooldown to be needed is
   * enabled and disabled. If multiple placces call this, the tracker will
   * automatically keep enabled until all places disable
   * 
  ...
  /**
   * Returns a number from 0 to 1 denoting the current cooldown amount, akin to
   ...
   * This counteracts rapid attacks via click macros, in a similar way to vanilla
   * by limiting to once every 10 ticks
   */
   ...
   * @param cooldownTime Relative cooldown time for the given source, 20 is
   *                     vanilla
   */
