...
public class JsonHelper {
  ...
  /**
   * Default GSON instance, use instead of creating a new instance unless you need
   * additional type adapaters
   */
  ...
  public static <T> T convertToEntry(Registry<T> registry, JsonElement element, String key) {
    ...
    throw new JsonSyntaxException("Unknown registry entry: " + name);
  }
  ...
  /**
   * @deprecated use
   *             {@link slimeknights.mantle.data.loadable.primitive.EnumLoadable}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * Gets a list of JSON objects for a single path in all domains and packs, for a
   * language file like loader
   */
  public static List<JsonObject> getFileInAllDomainsAndPacks(ResourceManager manager, String path,
      @Nullable String preferredPath) {
    return manager
        ...
        .flatMap(namespace -> {
          ...
          return manager.getResourceStack(location).stream()
              .map(preferredPath != null ? resource -> {
                Mantle.logger.warn("Using deprecated path {} in pack {} - use {}:{} instead", location,
                    resource.sourcePackId(), location.getNamespace(), preferredPath);
  ...
  private static void sendPackets(NetworkWrapper network, ServerPlayer player, ISimplePacket[] packets) {
    // on an integrated server, the modifier registries have a single instance on
    // both the client and the server thread
    // this means syncing is unneeded, and has the side-effect of recreating all the
    // modifier instances (which can lead to unexpected behavior)
    // as a result, integrated servers just mark fullyLoaded as true without syncing
    // anything, side-effect is listeners may run twice on single player
   ...
   * @param folder    Folder to trim (without trailing /), it is not validated so
   *                  make sure you call correctly
  ...
  /**
   * @deprecated use {@link BlockStateLoadable} with
   *             {@link slimeknights.mantle.data.loadable.Loadable#convert(JsonElement, String)}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * @deprecated use {@link BlockStateLoadable#DIFFERENCE} with
   *             {@link slimeknights.mantle.data.loadable.Loadable#getIfPresent(JsonObject, String)}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * @deprecated use {@link BlockStateLoadable#DIFFERENCE} with
   *             {@link slimeknights.mantle.data.loadable.record.RecordLoadable#deserialize(JsonObject)}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * @deprecated use {@link BlockStateLoadable#DIFFERENCE} with
   *             {@link slimeknights.mantle.data.loadable.Loadable#serialize(Object)}
   */
  @Deprecated(forRemoval = true)
  ...
  /**
   * @deprecated use {@link BlockStateLoadable#DIFFERENCE} with
   *             {@link slimeknights.mantle.data.loadable.record.RecordLoadable#serialize(Object, JsonObject)}
   */
  @Deprecated(forRemoval = true)
