...
public class ExtraHeartRenderHandler {
  ...
  public void renderHealthbar(RenderGuiLayerEvent.Pre event) {
    ...
    if (renderer == HeartRenderer.DISABLE || event.isCanceled()
    ...
    int maxHealth = Math.max(Mth.ceil(player.getAttributeValue(Attributes.MAX_HEALTH)),
        Math.max(displayHealth, health));
    int absorb = Mth.ceil(player.getAbsorptionAmount());

    // make hearts bounce with regen
    int regen = -1;
    if (player.hasEffect(MobEffects.REGENERATION)) {
      // vanilla uses max health here, but between us capping it and it looking
      // horrible at low sizes, use 25
      regen = tickCount % 25;
    }
    ...
    // if we have a large enough amount of health and its enabled, render the max
    // health as backgrounds
    if (renderer == HeartRenderer.NO_MAX || maxHealth <= 20) {
      ...
      absorptionOffset += 1;
      // all hearts for the container row are offset to the left by 1 for the sake of
      // doing -20 here. Means you don't see red until row 13
    ...
    // for absorption, render containers in same row if they fit, but never split
    // across rows (that gets confusing when we start stacking)
    if (absorb > 0) {
      ...
      if (compactAbsorption) {
        renderHeartRow(graphics, left + 8 * showHearts, top, 10, ABSORPTION_CONTAINER, container, 0, absorbHearts,
            half);
      } else {
        renderHeartRow(graphics, left, top - absorptionOffset, 10, ABSORPTION_CONTAINER, container, 0,
            Math.min(absorbHearts, 10), half);
      }
    }
  ...
  private void renderHearts(GuiGraphics graphics, int x, int y, int heartOffset, int count, int indexOffset) {
    ...
    renderHeartRow(graphics, x, y, indexOffset, colorOffset(heartIndex), heartOffset, 0, heartsTopColor,
        count % 2 == 1);
  }
  ...
  private void renderHeartsWithDamage(GuiGraphics graphics, int x, int y, int heartOffset, int current, int last) {
    ...
    if (damageWrapped) {
      ...
      // damage wrapping around means we cover up current middle to right, just a
      // question of whether we cover current entirely
      // ???__
      // ^^
      renderHeartRow(graphics, x, y, 0, colorOffset(last / 20 - 1), heartOffset + DAMAGE_OFFSET,
          bigDamage ? lastRight : currentRight, 10, false);
    } else {
      ...
      // damage taken did not wrap around, render it on top of lower current health
      // bar (rendering lower health if present)
      // *--##
      // ^^
      renderHeartRow(graphics, x, y, 0, colorOffset(last / 20), heartOffset + DAMAGE_OFFSET, currentRight, lastRight,
          lastTopRow % 2 == 1);
    }
    ...
    if (!bigDamage) {
      // ***?? OR --*??
      // ^^^ ^
      renderHeartRow(graphics, x, y, 0, colorOffset(current / 20), heartOffset, damageWrapped ? lastRight : 0,
          currentRight, currentTopRow % 2 == 1);
    }
    ...
    if (damageWrapped) {
      // --???
      // ^^
      renderHeartRow(graphics, x, y, 0, colorOffset(last / 20), heartOffset + DAMAGE_OFFSET, 0, lastRight,
          lastTopRow % 2 == 1);
    }
  }
   ...
   */
  private void renderHeartRow(GuiGraphics graphics, int x, int y, int indexOffset, int uOffset, int vOffset, int start,
      int end, boolean half) {
    // draw full hearts
    for (int i = start; i < end; i += 1) {
      graphics.blit(ICON_HEARTS, x + HEART_OFFSET * i, y + offsets[i + indexOffset], uOffset, vOffset, HEART_SIZE,
          HEART_SIZE);
    }
    // draw half heart
    if (half) {
      graphics.blit(ICON_HEARTS, x + HEART_OFFSET * end, y + offsets[end + indexOffset], uOffset + HEART_SIZE, vOffset,
  ...
  private void setOffsets(int indexOffset, int end, boolean wiggle, int regen) {
    for (int i = 0; i < end; i++) {
      // figure out the offset for this heart, and store it for future renderers to
      // check
      int offset = 0;
      if (wiggle) {
