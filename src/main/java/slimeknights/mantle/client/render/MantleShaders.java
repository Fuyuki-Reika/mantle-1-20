...
public class MantleShaders {
  ...
  /**
   * Gets the shader to use for {@link MantleRenderTypes#FLUID_SHADER}, checking
   * the config option to select which shader to use.
   */
  @Nullable
  ...
  static void registerShaders(RegisterShadersEvent event) throws IOException {
    event.registerShader(
        new ShaderInstance(event.getResourceProvider(), Mantle.getResource("block_fullbright"),
            DefaultVertexFormat.BLOCK),
        shader -> blockFullBrightShader = shader);
    event.registerShader(
        new ShaderInstance(event.getResourceProvider(), Mantle.getResource("fluid"),
