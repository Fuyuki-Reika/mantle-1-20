...
/**
 * Block model for setting color, luminosity, and per element uv lock. Similar
 * to {@link MantleItemLayerModel} but for blocks
 */
...
public class ColoredBlockModel extends SimpleBlockModel {
   ...
   * @param textures       List of textures for iteration, in case the owner is
   *                       not BlockModel
   ...
   * @param emissivity      Emissivity for fullbright, -1 will leave forge in
   *                        charge, 0-15 will override the forge value
   * @param spriteGetter    Sprite getter
   * @param transform       Transform for the face
   * @param quadTransformer Forge transformations for the face, this is notably
   *                        where you should handle color transformations
   * @param uvlock          UV lock for the face, separated to allow overriding
   *                        the model state
   * @param location        Model location
   */
  public static void bakePart(Builder builder, IGeometryBakingContext owner, BlockElement part, int emissivity,
      Function<Material, TextureAtlasSprite> spriteGetter, Transformation transform, IQuadTransformer quadTransformer,
      boolean uvlock, ResourceLocation location) {
  ...
  public static BakedModel bakeModel(IGeometryBakingContext owner, List<BlockElement> elements,
      List<ColorData> colorData, Function<Material, TextureAtlasSprite> spriteGetter, ModelState transform,
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (colors.luminosity != -1 && !location.equals(BAKE_LOCATION)) {
        Mantle.logger.warn(
            "Using deprecated 'luminosity' field on ColoredBlockModel color data for {}, this will be removed in 1.20 in favor of Forge's 'emissivity'.",
            location);
      }
      IQuadTransformer partTransformer = colors.color == -1 ? quadTransformer
          : quadTransformer.andThen(applyColorQuadTransformer(colors.color));
      bakePart(builder, owner, part, colors.luminosity, spriteGetter, transformation, partTransformer,
          colors.isUvLock(uvlock), location);
    }
  ...
  /**
   * Converts an ARGB color to an ABGR color, as the commonly used color format is
   * not the format colors end up packed into.
  ...
  /**
   * Extension of
   * {@code BlockModel#bakeFace(BlockPart, BlockPartFace, TextureAtlasSprite, Direction, IModelTransform, ResourceLocation)}
   * with emissivity and UV lock arguments
   * 
   ...
   * @param transform  Transform for the face
   * @param uvlock     UV lock for the face, separated to allow overriding the
   *                   model state
   * @param emissivity Emissivity for fullbright, -1 will leave forge in charge,
   *                   0-15 will override the forge value
   * @param location   Model location for errors
   */
  public static BakedQuad bakeFace(BlockElement part, BlockElementFace face, TextureAtlasSprite sprite,
      Direction facing, Transformation transform, boolean uvlock, int emissivity, ResourceLocation location) {
  ...
  /**
   * Extension of
   * {@link FaceBakery#bakeQuad(Vector3f, Vector3f, BlockElementFace, TextureAtlasSprite, Direction, ModelState, BlockElementRotation, boolean, ResourceLocation)}
   * with emissivity and UV lock overrides
   * 
  ...
  public static BakedQuad bakeQuad(Vector3f posFrom, Vector3f posTo, BlockElementFace face, TextureAtlasSprite sprite,
    ...
    // call the vanilla face bakery, we will pass in emmisivity and color via quad
    // transformers
    // note that in prior versions of mantle we reimplemented the face bakery
    // methods to pass in colors to the face baking directly
    // In 1.21, FaceBakery.bakeQuad() handles all vertex data setup including
    // normals
    Vector3f posFromVec = new Vector3f(posFrom.x(), posFrom.y(), posFrom.z());
    Vector3f posToVec = new Vector3f(posTo.x(), posTo.y(), posTo.z());
    BakedQuad tempQuad = FaceBakery.bakeQuad(posFromVec, posToVec, face, sprite, facing, transform, partRotation, shade,
        0);
    int[] vertexData = tempQuad.getVertices();
    Direction direction = tempQuad.getDirection();
