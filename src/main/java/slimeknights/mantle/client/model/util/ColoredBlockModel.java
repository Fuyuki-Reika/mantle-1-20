...
public class ColoredBlockModel extends SimpleBlockModel {
  ...
  public static BakedModel bakeModel(IGeometryBakingContext owner, List<BlockElement> elements,
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (colors.luminosity != -1) {
        Mantle.logger.warn(
            "Using deprecated 'luminosity' field on ColoredBlockModel color data, this will be removed in favor of Forge's 'emissivity'.");
      }
  ...
  /** Shared FaceBakery instance for baking faces */
  private static final FaceBakery FACE_BAKERY = new FaceBakery();

  /**
   * Extension of bakeFace with emissivity and UV lock arguments
   */
  public static BakedQuad bakeFace(BlockElement part, BlockElementFace face, TextureAtlasSprite sprite,
      Direction facing, Transformation transform, boolean uvlock, int emissivity) {
    return bakeQuad(part.from, part.to, face, sprite, facing, transform, uvlock, part.rotation, part.shade, emissivity);
  }

  /**
   * Extension of FaceBakery#bakeQuad with emissivity and UV lock overrides
   */
  public static BakedQuad bakeQuad(Vector3f posFrom, Vector3f posTo, BlockElementFace face, TextureAtlasSprite sprite,
    ...
    // call the vanilla face bakery, we will pass in emissivity and color via quad
    // transformers
    // FaceBakery.bakeQuad is now an instance method in 1.21.1
    Vector3f posFromVec = new Vector3f(posFrom.x(), posFrom.y(), posFrom.z());
    Vector3f posToVec = new Vector3f(posTo.x(), posTo.y(), posTo.z());
    // Create a minimal ModelState wrapping our Transformation (uvlock already
    // handled above)
    final Transformation xform = transform;
    BakedQuad tempQuad = FACE_BAKERY.bakeQuad(posFromVec, posToVec, face, sprite, facing,
        new ModelState() {
          @Override
    ...
    // use our override if specified, fallback to NeoForge face data
    if (emissivity == -1) {
