...
/**
 * Clone of {@link ItemLayerModel} to propagate a hardcoded color in, allows
 * reducing rendering time by bypassing item colors for a static color.
 * Also supports luminosity, and when used as a model loader supports telling a
 * layer to not use a tint index
 */
...
public class MantleItemLayerModel implements IUnbakedGeometry<MantleItemLayerModel> {
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker,
    ...
    TextureAtlasSprite particle = spriteGetter
        .apply(owner.hasMaterial("particle") ? owner.getMaterial("particle") : textures.get(0));
    ...
    for (int i = textures.size() - 1; i >= 0; i--) {
      ...
      quadBuilder.add(new QuadGroup(data.getRenderType(owner, normalRenderTypes),
          getQuadsForSprite(data.color(), data.noTint() ? -1 : i, sprite, transform, data.luminosity(), pixels)));
    }

    // build final model
    CompositeModel.Baked.Builder modelBuilder = CompositeModel.Baked.builder(owner, particle, overrides,
        owner.getTransforms());
   ...
   * @param tint       Tint index for
   *                   {@link net.minecraft.client.color.block.BlockColors} and
   *                   {@link net.minecraft.client.color.item.ItemColors}.
   *                   Generally unused
   * @param sprite     Sprite to convert into quads
   * @param transform  Transforms to apply
   * @param luminosity Extra light to add to the quad from 0-15, makes it appear
   *                   to glow a bit
   * @return List of baked quads
   */
  public static List<BakedQuad> getQuadsForSprite(int color, int tint, TextureAtlasSprite sprite,
   ...
   * @param sprite     Sprite to convert into quads
   * @param transform  Transforms to apply
   * @param emissivity Extra light to add to the quad from 0-15, makes it appear
   *                   to glow a bit
   * @param pixels     Object to keep track of used pixels across multiple layers
   *                   to help prevent z-fighting. To effective use, sprites must
   *                   be built in reverse order. Use null to skip this logic
   * @return List of baked quads
   * @see #getQuadForGui(int, int, TextureAtlasSprite, Transformation, int)
   */
  public static List<BakedQuad> getQuadsForSprite(int color, int tint, TextureAtlasSprite sprite,
      Transformation transform, int emissivity, @Nullable ItemLayerPixels pixels) {
      ...
      for (int v = 0; v < vMax; v++) {
        ...
        for (int u = 0; u < uMax; u++) {
          ...
          // make quad [uStart, u]
            ...
            // finish current quad if translucent (minimize overdraw) or we are forbidden
            // from touching this pixel (previous layer drew here)
            if (!canDraw || translucent) {
              int off = facing == Direction.DOWN ? 1 : 0;
              buildSideQuad(quadBuilder, quadConsumer, facing, color, sprite, uStart, v + off, uEnd - uStart,
                  emissivity);
              builder.add(quadBuilder.bakeQuad());
              building = false;
            }
          }
        }
      ...
      for (int u = 0; u < uMax; u++) {
        ...
        for (int v = 0; v < vMax; v++) {
          ...
          // make quad [vStart, v]
            ...
            // finish current quad if translucent (minimize overdraw) or we are forbidden
            // from touching this pixel (future layer drew here)
            if (!canDraw || translucent) {
              int off = facing == Direction.EAST ? 1 : 0;
              buildSideQuad(quadBuilder, quadConsumer, facing, color, sprite, u + off, vStart, vEnd - vStart,
                  emissivity);
              builder.add(quadBuilder.bakeQuad());
              building = false;
            }
    ...
    if (pixels != null) {
      ...
      // 1. if a pixel is only potentially there, don't draw lower layers - leads to
      // gaps
      // 2. if a pixel is only potentially there, always draw lower layers - leads to
      // z-fighting
      // 3. only use the first frame
      // of these, 2 would give the most accurate result. However, its also the
      // hardest to calculate
      // of the remaining methods, 3 is both more accurate and easier to calculate
      // than 1, so I opted for that approach
      if (hasFrames) {
        for (int v = 0; v < vMax; v++) {
          for (int u = 0; u < uMax; u++) {
  ...
  /**
   * Gets the quad to display in GUIs for the given sprite. Unlike
   * {@link #getQuadsForSprite(int, int, TextureAtlasSprite, Transformation, int, ItemLayerPixels)},
   * this method will not build side quads as GUIs don't see those.
   * This method is notably more efficient when you are only building a GUI model,
   * if you want a full model its probably more efficient to call the other method
   * then filter out {@link Direction#SOUTH} quads.
   * 
   * @param color      Color for the sprite in AARRGGBB format.
   * @param tint       Tint index for
   *                   {@link net.minecraft.client.color.block.BlockColors} and
   ...
   *                   to glow a bit
   * @return List of baked quads
   * @see #getQuadsForSprite(int, int, TextureAtlasSprite, Transformation, int,
   *      ItemLayerPixels)
   */
  @SuppressWarnings("unused") // API
  public static BakedQuad getQuadForGui(int color, int tint, TextureAtlasSprite sprite, Transformation transform,
      int emissivity) {
    // setup quad builder
    QuadBakingVertexConsumer quadBuilder = new QuadBakingVertexConsumer();
    // common settings
   ...
   * 
   * @param builder    Quad builder instance, just used to set direction so we can
   *                   avoid redundancy
   ...
   * @param u          Sprite U
   * @param v          Sprite V
   * @param size       Size of the quad in the correct direction (depth is always
   *                   1 pixel)
   * @param luminosity Extra light to add to the quad between 0 and 15
   */
  private static void buildSideQuad(QuadBakingVertexConsumer builder, VertexConsumer consumer, Direction side,
      int color, TextureAtlasSprite sprite, int u, int v, int size, int luminosity) {
    ...
    // for the side, Y axis's use of getOpposite is related to the swapping of V
    // direction
  ...
  /**
   * Builds a single quad in the model, based on the method in
   * {@link ItemLayerModel} but with color added
   * 
   * @param builder    Quad builder instance, just used to set direction so we can
   *                   avoid redundancy
   * @param consumer   Quad consumer, where we place the actual quad
   ...
   * @param luminosity Extra light to add to the quad between 0 and 15
   */
  public static void buildQuad(QuadBakingVertexConsumer builder, VertexConsumer consumer, Direction side, int color,
      int luminosity,
   ...
   * @param luminosity Extra light to add to the quad between 0 and 15
   */
  private static void putVertex(VertexConsumer consumer, Direction side, float x, float y, float z, float u, float v,
      int color, int luminosity) {
    // format is always DefaultVertexFormat#BLOCK, though order does not matter too
    // much
  ...
  public record LayerData(int color, int luminosity, boolean noTint, @Nullable ResourceLocation renderType) {
    ...
    /**
     * Gets the render type for this layer from the context, falling back to the
     * passed type if not requested
     */
